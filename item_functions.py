...
def cast_lighting(*args, **kwargs):
    ...
    if target:
        results.append({
            'consumed': True,
            'target': target,
            'message': Message('A light bolt strikes the {0} with loud thunder! The damage is {1}'.format(target.name,
                                                                                                          damage))
        })
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({
                'consumed': True,
                'message': Message('The eyes of the {0} look vacant, '
                                   'as he starts to stumble around!'.format(entity.name), libtcod.light_green)
            })
            break
    else:
        results.append({
